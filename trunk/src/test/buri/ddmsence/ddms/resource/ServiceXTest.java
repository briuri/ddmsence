 ...
 * @since 0.9.b
 */
public class ServiceXTest extends AbstractComponentTestCase {
	 ...
	 */
	private ServiceX testConstructor(boolean expectFailure, Element element) {
		ServiceX component = null;
		try {
			component = new ServiceX(TEST_PARENT_TYPE, element);
	 ...
	 */
	private ServiceX testConstructor(boolean expectFailure, List<String> names, List<String> phones, 
		List<String> emails) {
		ServiceX component = null;
		try {
			component = new ServiceX(TEST_PARENT_TYPE, names, phones, emails);
			checkConstructorSuccess(expectFailure);
		} catch (InvalidDDMSException e) {
			checkConstructorFailure(expectFailure, e);
		}
	...
	public void testNameAndNamespace() {
		for (String version : DDMSVersion.getSupportedVersions()) {
			...
			ServiceX component = testConstructor(WILL_SUCCEED, getValidElement(version));
			assertEquals(ServiceX.NAME, component.getName());
			assertEquals(PropertyReader.getProperty("ddms.prefix"), component.getPrefix());
			assertEquals(PropertyReader.getProperty("ddms.prefix") + ":" + ServiceX.NAME, component.getQualifiedName());
	...
	public void testElementConstructorValid() {
		for (String version : DDMSVersion.getSupportedVersions()) {
			...
			// No optional fields
			Element element = Util.buildDDMSElement(ServiceX.NAME, null);
	...
	public void testElementConstructorInvalid() {
		for (String version : DDMSVersion.getSupportedVersions()) {
			...
			// Empty name
			element = Util.buildDDMSElement(ServiceX.NAME, null);
	...
	public void testConstructorEquality() {
		for (String version : DDMSVersion.getSupportedVersions()) {
			DDMSVersion.setCurrentVersion(version);
			ServiceX elementComponent = testConstructor(WILL_SUCCEED, getValidElement(version));
			ServiceX dataComponent = testConstructor(WILL_SUCCEED, TEST_NAMES, TEST_PHONES, TEST_EMAILS);
	...
	public void testConstructorInequalityDifferentValues() {
		for (String version : DDMSVersion.getSupportedVersions()) {
			DDMSVersion.setCurrentVersion(version);
			ServiceX elementComponent = testConstructor(WILL_SUCCEED, getValidElement(version));
			ServiceX dataComponent = testConstructor(WILL_SUCCEED, TEST_NAMES, null, TEST_EMAILS);
	...
	public void testBuilder() throws InvalidDDMSException {
		for (String version : DDMSVersion.getSupportedVersions()) {
			...
			ServiceX.Builder builder = new ServiceX.Builder(component);
			assertEquals(builder.commit(), component);
			
			// Empty case
			builder = new ServiceX.Builder();
	...
	public void testBuilderLazyList() throws InvalidDDMSException {
		for (String version : DDMSVersion.getSupportedVersions()) {
			DDMSVersion.setCurrentVersion(version);
			ServiceX.Builder builder = new ServiceX.Builder();
